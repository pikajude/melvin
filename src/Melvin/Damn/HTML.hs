...
unescape :: Text -> Text
unescape text = case parseOnly message text of
    Left{..} -> text
    Right ms -> go ms
        where
            go (Numeric n:ns) = T.singleton (chr n) ++ go ns
            go (Named n:ns) = case M.lookup n namedEntities of
                Nothing -> "&" ++ n ++ ";" ++ go ns
                Just e -> T.singleton e ++ go ns
            go (Chunk t:ns) = t ++ go ns
            go [] = mempty

namedEntities :: Map Text Char
namedEntities =
